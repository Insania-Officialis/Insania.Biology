...
public class LoggingSL: ILoggingSL, IHostedService, IDisposable
{
    ...
    public LoggingSL(IServiceScopeFactory scopeFactory, ILogger<LoggingSL> logger)
    {
        ...
        //Создание канала
        _channel = Channel.CreateBounded<LogApiBiology>(options);
    }
    #endregion
    ...
    /// </summary>
    private readonly Channel<LogApiBiology> _channel;
    ...
    /// Пачка записей
    /// </summary>
    private readonly List<LogApiBiology> _batch = [];
    ...
    public ValueTask QueueLogAsync(LogApiBiology log) => _channel.Writer.WriteAsync(log, _cancellationTokenSource.Token);
    ...
    private async Task SaveBatchAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            LogsApiBiologyContext context = scope.ServiceProvider.GetRequiredService<LogsApiBiologyContext>();
